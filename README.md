...
> The `RiftwayLabsLooper` JSFX fork drives 5 loop columns from the Launchpad with LED
...
exposed state) we will use a **forked copy named `RiftwayLabsLooper`** — purely additive,
...
| [docs/03-architecture-proposal.md](docs/03-architecture-proposal.md) | Proposed RiftwayLabsLooper design, signal/MIDI routing, what gets built, MVP scope |
...
2. **No `js_ReaScriptAPI` / SWS / ReaImGui installed** — so a plain Lua script
   ...
   a forked **RiftwayLabsLooper JSFX** that emits MIDI-out to the device.
3. **DrivenByMoss already owns the `LPProMK3 MIDI` port** as a global control
   surface. It and RiftwayLabsLooper cannot share that port — see
...
3. Build order (from [docs/03-architecture-proposal.md](docs/03-architecture-proposal.md)):
   1. Back up `super8`, fork to `RiftwayLabsLooper` JSFX (LED out + per-track clear + note remap).
   2. Create the Reaper project template (input track, looper track, routing).
   3. Map Launchpad pads → RiftwayLabsLooper actions and utility pads → Reaper actions.
...
- **Additive only.** New files: `RiftwayLabsLooper` JSFX, new project template, new scripts.
